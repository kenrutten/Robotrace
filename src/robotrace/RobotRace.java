...
public class RobotRace extends Base {
    ...
    public void drawAxisFrame() {
        ...
        gl.glPushMatrix();
            glut.glutSolidSphere(0.15, 20, 20); //draw center sphere
            drawArrow(1,0,0); //draw arrow in x-direction
            drawArrow(0,1,0); //draw arrow in y-direction
            drawArrow(0,0,1); //draw arrow in z-direction
        gl.glPopMatrix();
     ...
     * @param z
     */
    public void drawArrow(double x, double y, double z) {
        gl.glColor3d(x*255,y*255,z*255); //define color based on axis
        gl.glTranslated(x*0.4, y*0.4, z*0.4); //translate by half the length of the cube
        gl.glScaled(0.1+x*0.7, 0.1+y*0.7, 0.1+z*0.7); //scale such that cube becomes elongated in x-, y- or z-direction
        glut.glutSolidCube(1);//draw cube
        gl.glScaled(1.25+(y+z)*8.75, 1.25+(x+z)*8.75, 1.25+(x+y)*8.75 );//reverse scaling
        gl.glTranslated(x*0.4, y*0.4, z*0.4);//translate to end of elongated cube, to draw cone at that spot
        gl.glRotated(90+z*270, -y, x, 1-x-y);//rotate cone
        drawCone();//draw cone
        gl.glRotated(-90-z*270, -y, x, 1-x-y);//reverse rotation
        gl.glTranslated(x*-0.8, y*-0.8, z*-0.8);//reverse translation
        

    }
    
    public void drawCone(){
       glut.glutSolidCone(0.12, 0.2, 50, 50);
    }
